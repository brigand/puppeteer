...
class ElementHandle extends JSHandle {
  ...
  async screenshot(options = {}) {
    ...
    const optionsToPass = Object.assign({}, options);
    delete optionsToPass.fullElement;

    const beforeScreenshot = async() => {
      let resetViewport = false;

      if (options.fullElement) {
        const { width: elWidth, height: elHeight} = await this.boundingBox();

        const width = Math.max(elWidth, viewport.width);
        const height = Math.max(elHeight, viewport.height);
      ...
      const optionsWithClip = Object.assign({}, optionsToPass, { clip });
      return { resetViewport, options: optionsWithClip };
    };

    optionsToPass._beforeScreenshot = beforeScreenshot;

    const res = await this._page.screenshot(optionsToPass);
